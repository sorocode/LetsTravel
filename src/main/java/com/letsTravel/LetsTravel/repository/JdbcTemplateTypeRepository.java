...
public class JdbcTemplateTypeRepository implements TypeRepository {
	...
	public int modifyTypeNameTranslated(PrimaryTypeDetailDTO PrimaryTypeDetailDTO) {
		...
		String sql = "UPDATE Type SET Type_name_translated = ? WHERE Type_seq = (SELECT temp.Type_seq FROM(SELECT Type_seq FROM Type WHERE Type_name = ? AND Type_name_translated IS NULL) temp);";
